#!/bin/bash
    ...
    detect_os ()
  {
      ...
      finalize_yum_repo ()
    {
      ...
      yum install -y pygpgme --disablerepo='rabbitmq_rabbitmq-server'
      ...
      echo "More information: https://fedoraproject.org/wiki/EPEL#How_can_I_use_these_extra_packages.3F"
        ...
        # set the repo_gpgcheck option to 0
                                sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/rabbitmq_rabbitmq-server.repo
                                fi
                                fi
                                
                                echo "Installing yum-utils..."
                                yum install -y yum-utils --disablerepo='rabbitmq_rabbitmq-server'
    ...
    echo "The yum-utils package could not be installed. This means you may not be able to install source RPMs or use other yum features."
                                ...
                                echo "Generating yum cache for rabbitmq_rabbitmq-server..."
                                yum -q makecache -y --disablerepo='*' --enablerepo='rabbitmq_rabbitmq-server'
                                
                                echo "Generating yum cache for rabbitmq_rabbitmq-server-source..."
                                yum -q makecache -y --disablerepo='*' --enablerepo='rabbitmq_rabbitmq-server-source'
    }
      ...
      main ()
    {
      ...
      yum_repo_path=/etc/zypp/repos.d/rabbitmq_rabbitmq-server.repo
      else
      yum_repo_path=/etc/yum.repos.d/rabbitmq_rabbitmq-server.repo
      fi
